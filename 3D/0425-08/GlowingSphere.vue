...
const props = defineProps({
  sphereColor: { type: [String, Number], default: 0x3399ff },
  wireframeColor: { type: [String, Number], default: 0x88ddff },
  starColor: { type: [String, Number], default: 0xffdd00 },
  backgroundColor: { type: [String, Number], default: 0x0a0a1a },
  sphereOpacity: { type: Number, default: 0.6 },
  waveSpeed: { type: Number, default: 2.0 },
  waveAmplitude: { type: Number, default: 0.08 },
  showStars: { type: Boolean, default: true },
  showWireframe: { type: Boolean, default: true },
...
function createStarsOnSphere() {
  ...
  const starMaterial = new THREE.LineBasicMaterial({
    color: parseColor(props.starColor),
    transparent: true,
    opacity: 0.9
  });
...
function createLatitudeLine(latitudeIndex, totalLatitudes, baseRadius) {
  ...
  const material = new THREE.LineBasicMaterial({
    color: parseColor(props.wireframeColor),
    transparent: true,
    opacity: 0.8
  });
...
function initScene() {
  ...
  try {
    ...
    scene.background = new THREE.Color(parseColor(props.backgroundColor));
    ...
    const sphereMaterial = new THREE.MeshPhysicalMaterial({
      color: parseColor(props.sphereColor),
      transparent: true,
      opacity: props.sphereOpacity,
      transmission: 0.3,
      roughness: 0.1,
      metalness: 0.1,
      clearcoat: 0.8,
    ...
    emit('ready', { scene, camera, renderer, sphere });
  } catch (error) {
    console.error('初始化 GlowingSphere 失败:', error);
    emit('error', error);
  }
}
...
function updateLatitudeWave(time) {
  ...
  const waveAmplitude = props.waveAmplitude;
...
function updateLongitudeWave(time) {
  const waveSpeed = props.waveSpeed * 0.75;
  const waveAmplitude = props.waveAmplitude * 0.625;
...
function animate() {
  ...
  if (sphere && props.autoRotate) {
    sphere.rotation.y = time * 0.1;
    sphere.rotation.x = Math.sin(time * 0.1) * 0.05;
    sphere.material.opacity = props.sphereOpacity * (0.83 + Math.sin(time * 1.5) * 0.17);
  }
...
function dispose() {
  ...
  [...latitudeLines, ...longitudeLines].forEach(line => {
...
watch(() => props.sphereColor, (newValue) => {
  if (sphere) {
    sphere.material.color.setHex(parseColor(newValue));
  }
});

watch(() => props.wireframeColor, () => {
  dispose();
  initScene();
});

watch(() => props.starColor, () => {
  dispose();
  initScene();
});
...
watch(() => props.backgroundColor, (newValue) => {
  if (scene) {
    scene.background = new THREE.Color(parseColor(newValue));
  }
});

watch(() => props.sphereOpacity, (newValue) => {
  if (sphere) {
    sphere.material.opacity = newValue;
  }
});

watch(() => [props.showStars, props.showWireframe], () => {
  dispose();
  initScene();
});
